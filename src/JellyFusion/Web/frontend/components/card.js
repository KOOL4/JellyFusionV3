...
(function () {
    ...
    JF.Card = function (opts) {
        ...
        // v3.0.30: theme can inject extra content above the title.
        // Used e.g. by the Netflix theme to render the "98% Match"
        // badge. When no theme is active or the theme didn't
        // implement the hook, JF.themes.invoke returns undefined
        // and we just skip the prepend.
        if (JF.themes && typeof JF.themes.invoke === 'function') {
            var extras = JF.themes.invoke('cardOverlayExtras', [item]);
            if (typeof extras === 'string' && extras) {
                var extrasNode = document.createElement('div');
                extrasNode.className = 'jf-card__overlay-extras';
                extrasNode.innerHTML = extras;
                overlay.appendChild(extrasNode);
            }
        }
