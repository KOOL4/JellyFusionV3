...
(function () {
    ...
    // ---------------------------------------------------------
    //  4b. Item playback helper (v3.0.15)
    //
    //  v3.0.14 tried the AMD playbackManager first but require()
    //  resolution from outside the SPA module loader was unreliable -
    //  on the user's setup the click silently no-op'd. v3.0.15 inverts
    //  the order: navigate to the canonical Jellyfin video URL first
    //  (which the SPA's router DOES handle and starts playback), and
    //  only try playbackManager if URL nav fails.
    //
    //  Jellyfin's video URL is `#/video?id=X&serverId=Y`. The SPA
    //  matches that route and bootstraps the player straight to play.
    // ---------------------------------------------------------
    function playItem(itemId) {
        ...
        try {
            ...
            try {
                ...
                } else if (window.ApiClient && window.ApiClient._serverInfo && window.ApiClient._serverInfo.Id) {
                    serverId = window.ApiClient._serverInfo.Id;
                }
            } catch (e) {}

            // Primary path: canonical video URL. The SPA picks this up
            // and starts playback immediately on every Jellyfin web
            // version we've tested (10.10 LTS through 10.11.8).
            var url = "/video?id=" + encodeURIComponent(itemId);
            if (serverId) url += "&serverId=" + encodeURIComponent(serverId);
            location.hash = url;
            log("playItem -> " + url);

            // Belt-and-suspenders: ALSO try playbackManager via
            // require AFTER a tick, so on web clients that don't
            // route #/video directly to playback the manager kicks in.
            setTimeout(function () {
                        ...
                        window.require(["playbackManager"], function (pm) {
                            try {
                                if (pm && typeof pm.play === "function") {
                                    pm.play({ ids: [itemId], serverId: serverId });
                                }
                            } catch (e) { /* swallow */ }
                        });
                    } catch (e) {}
                } else if (window.playbackManager && typeof window.playbackManager.play === "function") {
                    try { window.playbackManager.play({ ids: [itemId], serverId: serverId }); }
                    catch (e) {}
                }
            }, 250);
        } catch (e) {
            warn("playItem failed, falling back to details:", e && e.message);
