...
(function () {
    ...
    // ---------------------------------------------------------
    //  4b. Item playback helper (v3.0.14)
    //
    //  Tries the standard Jellyfin playbackManager first; falls back
    //  to URL-based navigation. Never throws.
    // ---------------------------------------------------------
    function playItem(itemId) {
        try {
            var serverId = (window.ApiClient && typeof window.ApiClient.serverId === "function")
                ? window.ApiClient.serverId() : null;

            // (1) Canonical: AMD-loaded playbackManager.
            if (typeof window.require === "function") {
                try {
                    window.require(["playbackManager"], function (pm) {
                        ...
                        fallbackPlay(itemId, serverId);
                    });
                    return;
                } catch (e) { /* fall through */ }
            }

            // (2) Some forks attach playbackManager directly to window.
            if (window.playbackManager && typeof window.playbackManager.play === "function") {
                window.playbackManager.play({ ids: [itemId], serverId: serverId });
                return;
            }
            ...
            warn("playItem failed:", e && e.message);
            location.hash = "#/details?id=" + itemId;
        }
    }

    function fallbackPlay(itemId, serverId) {
    ...
    function renderTopNav() {
        ...
        fetchJson("/jellyfusion/navigation").then(function (items) {
