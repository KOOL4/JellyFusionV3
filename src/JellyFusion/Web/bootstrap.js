...
(function () {
    ...
    function injectBaseStyles() {
        ...
        var css = [
            ...
            // v3.0.25: rail layout polished — header row with title +
            // arrows on the right, larger card baseline (200px), nicer
            // hover transform, scrollbar hidden (we have arrows now).
            ".jf-rail{margin:24px 0;position:relative;}",
            ".jf-rail-header{display:flex;align-items:center;justify-content:space-between;",
            "padding:0 2%;margin-bottom:10px;}",
            ".jf-rail-header h2{font-size:18px;font-weight:700;margin:0;color:#fff;letter-spacing:.01em;}",
            ".jf-rail-nav{display:flex;gap:6px;opacity:0;transition:opacity .25s ease;}",
            ".jf-rail:hover .jf-rail-nav{opacity:1;}",
            ".jf-rail-arrow{background:rgba(0,0,0,.55);backdrop-filter:blur(20px);",
            "border:1px solid rgba(255,255,255,.1);color:#fff;width:34px;height:34px;",
            "border-radius:50%;cursor:pointer;font-size:22px;line-height:1;padding:0;",
            "display:flex;align-items:center;justify-content:center;",
            "transition:all .15s ease;}",
            ".jf-rail-arrow:hover{background:rgba(255,255,255,.18);border-color:rgba(255,255,255,.25);",
            "transform:scale(1.08);}",
            ".jf-rail-arrow.jf-arrow-disabled{opacity:.25;pointer-events:none;}",
            ".jf-rail .jf-row{display:flex;gap:12px;overflow-x:auto;padding:4px 2% 18px;",
            "scrollbar-width:none;scroll-behavior:smooth;}",
            ".jf-rail .jf-row::-webkit-scrollbar{display:none;}",
            // Cards: bigger baseline (200px), softer corners, lift on
            // hover with a subtle ring + shadow, image scale subtly.
            ".jf-rail .jf-card{flex:0 0 auto;width:200px;position:relative;border-radius:10px;",
            "overflow:hidden;background:#15171c;cursor:pointer;",
            "transition:transform .25s ease,box-shadow .25s ease;}",
            ".jf-rail .jf-card:hover{transform:translateY(-4px) scale(1.04);",
            "box-shadow:0 12px 30px rgba(0,0,0,.55),0 0 0 2px rgba(0,164,220,.4);z-index:2;}",
            ".jf-rail .jf-card img{width:100%;aspect-ratio:2/3;object-fit:cover;display:block;",
            "transition:filter .25s ease;}",
            ".jf-rail .jf-card:hover img{filter:brightness(1.06);}",

            // v3.0.26: 4 card formats (matches the 'home screen
            // sections' Jellyfin plugin the user referenced).
            //   portrait  - default 2:3, 200px wide
            //   landscape - 16:9 wide cards (was 'horizontal' in v3.0.23)
            //   square    - 1:1 thumbnails
            //   small     - compact 2:3, 140px wide (more cards visible)
            ".jf-rail.jf-format-portrait .jf-card{width:200px;aspect-ratio:2/3;}",
            ".jf-rail.jf-format-portrait .jf-card img{aspect-ratio:2/3;}",
            ".jf-rail.jf-format-landscape .jf-card{width:300px;aspect-ratio:16/9;}",
            ".jf-rail.jf-format-landscape .jf-card img{aspect-ratio:16/9;}",
            // legacy alias - rails created in v3.0.23 used 'horizontal'
            // as the format name; map it to the new landscape rules so
            // existing configs keep rendering correctly without a
            // migration step.
            ".jf-rail.jf-format-horizontal .jf-card{width:300px;aspect-ratio:16/9;}",
            ".jf-rail.jf-format-horizontal .jf-card img{aspect-ratio:16/9;}",
            ".jf-rail.jf-format-square .jf-card{width:200px;aspect-ratio:1/1;}",
            ".jf-rail.jf-format-square .jf-card img{aspect-ratio:1/1;}",
            ".jf-rail.jf-format-small .jf-card{width:140px;aspect-ratio:2/3;}",
            ".jf-rail.jf-format-small .jf-card img{aspect-ratio:2/3;}",
            ".jf-rail.jf-format-small .jf-card .jf-name{font-size:11px;padding:4px 6px;}",

            // v3.0.26: NETFLIX TOP 10 — huge translucent number BESIDE
            // each poster (matches the user's Apple TV+ / Netflix
            // mobile reference). Number height matches card height.
            // The .jf-top10-item flex wrapper holds [number][poster].
            // -webkit-text-stroke for the cyan ring around the number;
            // text-shadow gives it depth like a frosted glass digit.
            ".jf-rail-top10 .jf-row{align-items:center;}",
            ".jf-rail-top10 .jf-top10-item{display:flex;align-items:center;flex:0 0 auto;}",
            ".jf-rail-top10 .jf-top10-item .jf-card{flex:0 0 auto;}",
            ".jf-rail-top10 .jf-rank-big{",
            "font:900 200px/1 'Arial Black','Helvetica Neue',sans-serif;",
            "color:rgba(255,255,255,.06);",
            "-webkit-text-stroke:3px rgba(255,255,255,.65);",
            "text-shadow:0 8px 30px rgba(0,0,0,.6),inset 0 0 30px rgba(255,255,255,.2);",
            "margin-right:-30px;line-height:.85;letter-spacing:-.05em;",
            "user-select:none;pointer-events:none;",
            "background:linear-gradient(180deg,rgba(255,255,255,.18) 0%,rgba(255,255,255,0) 60%);",
            "-webkit-background-clip:text;background-clip:text;",
            "z-index:1;}",
            // Smaller number for landscape/small formats.
            ".jf-rail-top10.jf-format-landscape .jf-rank-big{font-size:140px;margin-right:-20px;}",
            ".jf-rail-top10.jf-format-small .jf-rank-big{font-size:140px;margin-right:-20px;}",
            ...
            // v3.0.26: legacy .jf-rank (small white number on the
            // poster) is gone - the new .jf-rank-big (huge number
            // BESIDE the poster) replaces it for Top 10 rails.
    ...
    // v3.0.24: subscribe ONCE to window message events from any
    // YouTube iframe we mount. The IFrame Player API (enabled by
    // ?enablejsapi=1 on the iframe URL) sends JSON-encoded events
    // here. We care about onError specifically — error codes 100
    // (video not found), 101/150 (embedding disabled by owner),
    // 153 (player config error). When ANY of those arrive, kill
    // the iframe so the user sees the still photo instead of the
    // YouTube error page. We also send the magic "listening" frame
    // back to the iframe so YouTube actually starts emitting events.
    if (!window.__JF_TRAILER_LISTENER__) {
        window.__JF_TRAILER_LISTENER__ = true;
        window.addEventListener("message", function (ev) {
            if (!trailerState.iframe) return;
            // Origin can be either www.youtube.com or www.youtube-nocookie.com
            // depending on the embed URL; accept both.
            var ok = ev.origin === "https://www.youtube.com" ||
                     ev.origin === "https://www.youtube-nocookie.com";
            if (!ok) return;
            try {
                var data = typeof ev.data === "string" ? JSON.parse(ev.data) : ev.data;
                if (!data) return;
                if (data.event === "onError" ||
                    (data.info && (data.info.errorCode === 150 ||
                                   data.info.errorCode === 101 ||
                                   data.info.errorCode === 153 ||
                                   data.info.errorCode === 100))) {
                    warn("trailer iframe reported onError - removing", data);
                    cancelTrailerOverlay();
                }
    ...
    function scheduleTrailerOverlay(itemId) {
        ...
        cancelTrailerOverlay();
        // Skip if autoplay interval is too short - the iframe wouldn't
        // even finish loading before we cut to the next slide.
        var interval = (bannerState.autoplayInterval || 7);
        if (bannerState.autoplay !== false && interval < 5) return;

        // 2 SECONDS still image → trailer fades in over the next ~1.4s
        // → user sees motion at second 3 (matches the user's spec exactly).
        trailerState.timer = setTimeout(function () {
            fetchJson("/jellyfusion/slider/trailer/" + encodeURIComponent(itemId))
                .then(function (resp) {
                    ...
                    if (!bannerState.el) return;          // banner torn down

                    // v3.0.22: append &origin= so YouTube can satisfy
                    // the iframe-postMessage origin check. Without it,
                    // the player sometimes reports Error 153 (config
                    // error) on hosts where the embed isn't whitelisted.
                    var src = resp.embedUrl;
                    try {
                    ...
                    iframe.style.cssText =
                        ...
                        "border:0;opacity:0;transition:opacity 1s ease;pointer-events:none;" +
                        "z-index:2;";
                    bannerState.el.appendChild(iframe);
                    trailerState.iframe = iframe;

                    // v3.0.22: tear down if YouTube fails to load.
                    var loadOk = false;
                    iframe.addEventListener("load", function () {
                        loadOk = true;
                        // v3.0.24: subscribe to YouTube IFrame API events
                        // by sending the magic "listening" frame to the
                        // iframe. YouTube responds with onReady/onError/
                        // onStateChange messages we listen for at the
                        // window level (see __JF_TRAILER_LISTENER__).
                        try {
                            iframe.contentWindow.postMessage(JSON.stringify({
                                event: "listening",
                                id:    "jellyfusion-trailer",
                                channel: "widget"
                            }), "*");
                        } catch (e) {}
                    });
                    setTimeout(function () {
                        if (!loadOk && trailerState.iframe === iframe) {
                            warn("trailer iframe never reported load - removing");
                            cancelTrailerOverlay();
                        }
                    }, 4000);

                    // Fade in next tick so the transition runs.
                    setTimeout(function () { iframe.style.opacity = "1"; }, 60);
                })
                .catch(function (e) { /* no trailer, no problem */ });
        }, 2000);  // exactly 2s still
    }
    ...
    function renderRails(container) {
        fetchJson("/jellyfusion/home/rails").then(function (rails) {
            ...
            rails.forEach(function (rail) {
                ...
                // v3.0.23: append jf-format-{poster|horizontal} so the
                // CSS picks the right card aspect-ratio (2:3 for poster,
                // 16:9 for horizontal "recently added" style rows).
                var fmt = (rail.format || "Poster").toLowerCase();
                row.className = "jf-rail jf-rail-" + (rail.id || "default") + " jf-format-" + fmt;
                row.setAttribute("data-jf-rail", rail.id || rail.title || "rail");
                row.setAttribute("data-jf-format", fmt);

                // v3.0.26: detect Top 10 layout. Top 10 rails get the
                // big translucent number rendered BESIDE each poster
                // (Netflix-mobile style). The container gets a class so
                // CSS knows to lay out card+number side-by-side.
                if (rail.showRank) row.classList.add("jf-rail-top10");

                var cards = rail.items.map(function (it, idx) {
                    // v3.0.26: for Top 10 rails the rank is rendered
                    // OUTSIDE the .jf-card wrapper so the number can be
                    // huge (full card height) without being clipped by
                    // the card's overflow:hidden.
                    var rank = rail.showRank
                        ? "<div class='jf-rank-big'>" + (idx + 1) + "</div>"
                        : "";
                    ...
                    // v3.0.26: 4 formats now — portrait (2:3), landscape
                    // (16:9 backdrop), square (1:1), small (compact 2:3).
                    // Image source matches the format: landscape wants
                    // a backdrop, the others want the vertical poster.
                    var img = (fmt === "landscape" || fmt === "horizontal")
                        ? "/Items/" + it.id + "/Images/Backdrop?fillHeight=270&quality=80"
                        : "/Items/" + it.id + "/Images/Primary?fillHeight=270&quality=80";
                    // For Top 10 rails the rank stands BESIDE the poster
                    // wrapped in .jf-top10-item so flexbox can lay them
                    // out side-by-side. For everything else we keep the
                    // simple .jf-card structure.
                    if (rail.showRank) {
                        return "<div class='jf-top10-item'>" +
                                   rank +
                                   "<div class='jf-card' data-id='" + it.id + "'>" +
                ...
                // v3.0.25: rail header with title + scroll arrows on
                // the right (Jellyfin / Netflix style). Arrows fade in
                // on rail hover and scroll the row by ~80% of the
                // visible width with smooth behaviour.
                row.innerHTML =
                    "<div class='jf-rail-header'>" +
                        "<h2>" + (rail.title || "") + "</h2>" +
                        "<div class='jf-rail-nav'>" +
                            "<button type='button' class='jf-rail-arrow jf-rail-prev' aria-label='Anterior'>&#8249;</button>" +
                            "<button type='button' class='jf-rail-arrow jf-rail-next' aria-label='Siguiente'>&#8250;</button>" +
                        "</div>" +
                    "</div>" +
                ...
                row.addEventListener("click", function (ev) {
                    // v3.0.25: ignore clicks that came from the nav
                    // arrows so they don't cascade into a card click.
                    if (ev.target.closest && ev.target.closest(".jf-rail-arrow")) return;
                ...
                // v3.0.25: wire up the prev/next arrows to scroll the
                // row by ~80% of its viewport. We re-evaluate clientWidth
                // on each click so the scroll distance follows window
                // resizing. updateArrowsState() hides the prev button at
                // scrollLeft==0 and the next button at end-of-scroll so
                // the user gets natural visual feedback.
                var rowEl = row.querySelector(".jf-row");
                var prevBtn = row.querySelector(".jf-rail-prev");
                var nextBtn = row.querySelector(".jf-rail-next");
                function scrollAmount() { return Math.max(rowEl.clientWidth * 0.85, 320); }
                ...
                function updateArrowsState() {
                    var atStart = rowEl.scrollLeft <= 4;
                    var atEnd   = rowEl.scrollLeft + rowEl.clientWidth >= rowEl.scrollWidth - 4;
                    if (prevBtn) prevBtn.classList.toggle("jf-arrow-disabled", atStart);
                    if (nextBtn) nextBtn.classList.toggle("jf-arrow-disabled", atEnd);
                }
                rowEl.addEventListener("scroll", updateArrowsState, { passive: true });
