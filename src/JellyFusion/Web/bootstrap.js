        ...
        trailerState.timer = setTimeout(function () {
            fetchJson("/jellyfusion/slider/trailer/" + encodeURIComponent(itemId))
                .then(function (resp) {
                    ...
                    // v3.0.22: tear down if YouTube fails to load (the
                    // Error 153 page would otherwise stay on screen).
                    // We can't read the iframe contents (CORS), so we
                    // poll: if the iframe stays at offsetHeight==0 or
                    // contentDocument throws an unexpected load error
                    // after 4s, kill it and let the still photo win.
                    var loadOk = false;
                    iframe.addEventListener("load", function () { loadOk = true; });
                    setTimeout(function () {
