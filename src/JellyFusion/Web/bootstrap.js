...
(function () {
    ...
    function injectBaseStyles() {
        ...
        var css = [
            ...
            ".jf-style-disney{border-radius:24px;height:62vw;max-height:640px;",
            "background:linear-gradient(180deg,#0b1220 0%,#020617 100%);",
            "padding-top:64px;box-shadow:0 20px 60px rgba(0,0,0,.7);}",
            ...
            // ===== Apple TV+ STYLE =====
            // Fullscreen-feel banner with FEATHERED edges (mask-image
            // gradient on all 4 sides) so the still blends into the
            // page background, like the Apple TV+ home. Small "Apple TV+"
            // pill in top-left for branding.
            ".jf-style-apple{border-radius:0;height:62vw;max-height:680px;background:transparent;",
            "box-shadow:none;margin:0 0 32px;padding-top:0;}",
            ".jf-style-apple .jf-bg{filter:brightness(.85);",
            "-webkit-mask-image:radial-gradient(ellipse 100% 100% at 50% 50%,#000 55%,transparent 95%);",
            "mask-image:radial-gradient(ellipse 100% 100% at 50% 50%,#000 55%,transparent 95%);}",
            ".jf-style-apple .jf-grad{background:linear-gradient(180deg,",
            "rgba(0,0,0,.0) 0%,rgba(0,0,0,.0) 50%,rgba(0,0,0,.45) 80%,rgba(0,0,0,.85) 100%);}",
            ".jf-style-apple .jf-content{left:6%;bottom:8%;max-width:48%;}",
            ".jf-style-apple .jf-title{font-family:'New York',Georgia,serif;font-style:italic;",
            "font-size:clamp(40px,5.5vw,82px);font-weight:700;letter-spacing:-.03em;line-height:1;",
            "margin-bottom:14px;text-shadow:0 6px 24px rgba(0,0,0,.8);}",
            ".jf-style-apple .jf-meta{display:inline-block;background:rgba(40,40,40,.7);backdrop-filter:blur(20px);",
            "padding:5px 12px;border-radius:6px;font-size:13px;color:#fff;margin-bottom:10px;}",
            ".jf-style-apple .jf-desc{color:#e5e5e5;font-size:16px;}",
            ".jf-style-apple .jf-cta .jf-primary{background:#1d1d1f;color:#fff;padding:10px 32px;",
            "border-radius:30px;border:1px solid rgba(255,255,255,.18);font-weight:600;font-size:15px;",
            "backdrop-filter:blur(20px);}",
            ".jf-style-apple .jf-cta .jf-primary:hover{background:#2a2a2c;}",
            ".jf-style-apple .jf-cta .jf-ghost{background:rgba(40,40,40,.7);border:1px solid rgba(255,255,255,.18);",
            "padding:10px 18px;border-radius:30px;backdrop-filter:blur(20px);}",
            ".jf-style-apple .jf-dots{left:50%;right:auto;transform:translateX(-50%);bottom:18px;}",
            ".jf-style-apple .jf-dots span{background:rgba(255,255,255,.35);}",
            ".jf-style-apple .jf-dots span.active{background:#fff;width:6px;height:6px;}",
    ...
    function renderBanner(container) {
        fetchJson("/jellyfusion/slider/items").then(function (response) {
            ...
            var items, style, autoplay = true, autoplayInterval = 7;
            ...
            banner.className = "jf-banner jf-style-" + style;
            banner.setAttribute("data-jf-style", style);
            banner.id        = "jf-banner";

            // Disney+ style: render decorative platform tabs at the top
            // (purely visual - clicking a tab cycles to a slide).
            // Apple TV+ style: render a small "Apple TV+" pill in the
            // top-left corner for branding.
            var topRow = "";
            if (style === "disney") {
                topRow =
                    "<div class='jf-disney-tabs'>" +
                    "  <span class='jf-tab active'>Para ti</span>" +
                    "  <span class='jf-tab'>Películas</span>" +
                    "  <span class='jf-tab'>Series</span>" +
                    "  <span class='jf-tab'>Estudios</span>" +
                    "</div>";
            } else if (style === "apple") {
                topRow =
