...
public class JellyFusionController : ControllerBase
{
    ...
    public IActionResult GetNavigation()
    {
        try
        {
            ...
            string? FindLibUrl(params string[] candidateNames)
            {
                foreach (var name in candidateNames)
                {
                    var hit = libs.FirstOrDefault(l =>
                        string.Equals(l.Name, name, StringComparison.OrdinalIgnoreCase));
                    if (hit is not null)
                        return $"#/list.html?parentId={hit.Id:N}";
                }
                return null;
            }
            ...
            string ResolveUrl(Configuration.NavItem n)
            {
                ...
                return n.Id switch
                {
                    ...
                    "movies"    => FindLibUrl("Películas", "Movies", "Pelis", "Cine") ?? "#/home.html",
                    "series"    => FindLibUrl("Series", "TV Shows", "Shows") ?? "#/home.html",
                    "studios"   => "#/home.html",
                    "live"      => "#/livetv.html",
                    _           => "#/home.html"
                };
