...
public class JellyFusionController : ControllerBase
{
    ...
    public IActionResult GetNavigation()
    {
        try
        {
            ...
            // can also surface UI flags the bootstrap needs (currently
            // only HideStockMyMedia, but keeping the shape extensible).
            // Bootstrap normalises raw-array AND object shapes for
            // backwards compatibility with v3.0.14 cached scripts.
            var hideStock = Plugin.Instance?.Configuration?.Home?.HideStockMyMedia ?? false;
            return Ok(new
            {
