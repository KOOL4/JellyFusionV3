...
public class JellyFusionController : ControllerBase
{
    ...
    /// <summary>
    /// v3.0.27: serves files from <c>Web/frontend/**</c> as embedded
    /// resources. Lets bootstrap.js (or a hand-pasted devtools loader)
    /// fetch <c>/jellyfusion/frontend/app.js</c>,
    /// <c>/jellyfusion/frontend/components/card.js</c>, etc., without
    /// adding a new IStaticAssetsProvider or messing with Jellyfin's
    /// /web/ static handler.
    ///
    /// Path is sanitised by replacing both forward and back slashes
    /// with dots (matches the embedded-resource naming convention)
    /// and rejecting any segment that contains "..". MIME type comes
    /// from the file extension; defaults to text/plain so unknown
    /// extensions don't break the download.
    /// </summary>
    [HttpGet("frontend/{**path}")]
    [AllowAnonymous]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public IActionResult GetFrontendAsset(string path)
    {
        if (string.IsNullOrEmpty(path) || path.Contains("..", StringComparison.Ordinal))
            return NotFound();

        // "components/card.js"  -> "components.card.js"
        var resourceTail = path.Replace('/', '.').Replace('\\', '.');
        var resourceName = typeof(Plugin).Namespace + ".Web.frontend." + resourceTail;

        var asm    = typeof(Plugin).Assembly;
        var stream = asm.GetManifestResourceStream(resourceName);
        if (stream is null)
        {
            _logger.LogDebug("Frontend asset not found: {Resource}", resourceName);
            return NotFound();
        }

        // Pick the right MIME by extension. Cache short so updates
        // during development land without a hard refresh; the bust
        // query string in app.js is still needed for stylesheet swaps.
        string contentType = path.EndsWith(".js",   StringComparison.OrdinalIgnoreCase) ? "application/javascript"
                           : path.EndsWith(".css",  StringComparison.OrdinalIgnoreCase) ? "text/css"
                           : path.EndsWith(".html", StringComparison.OrdinalIgnoreCase) ? "text/html"
                           : path.EndsWith(".json", StringComparison.OrdinalIgnoreCase) ? "application/json"
