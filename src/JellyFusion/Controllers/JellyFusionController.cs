...
///
/// v3.0.7 AUTH OVERHAUL:
///   Previous versions put `[Authorize(Policy="RequiresElevation")]` on the
///   class and `[AllowAnonymous]` on the endpoints that bootstrap.js hits.
///   That was wrong: with [AllowAnonymous] the ASP.NET Core auth pipeline
///   doesn't run at all, so even when the browser DID send the X-Emby-Token
///   cookie, User.Claims was empty and GetUserId() returned null. All
///   user-scoped rails (continueWatching, becauseYouWatched, etc.) then
///   resolved to empty arrays, the bootstrap dropped them as "no items",
///   and the home showed a banner with NOTHING below it - which matches
///   exactly the v3.0.6 feedback.
///
///   Per-endpoint auth now:
///     - config/*           -> RequiresElevation (admin only)
///     - bootstrap.js,
///       theme/css, i18n   -> AllowAnonymous (no user context needed)
///     - home/rails,
///       slider/items,
///       studios           -> DefaultAuthorization (populates User.Claims so
///                            GetUserId works, and falls back to first
///                            admin when the claim is unexpectedly missing)
/// </summary>
[ApiController]
[Route("jellyfusion")]
