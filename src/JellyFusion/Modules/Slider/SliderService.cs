...
public class SliderService
{
        ...
        if (user is null)
        {
            try
            {
                user = _userManager.Users
                    .FirstOrDefault(u => u.HasPermission(Jellyfin.Data.Enums.PermissionKind.IsAdministrator))
                    ?? _userManager.Users.FirstOrDefault();
                if (user is not null)
                {
                    _logger.LogDebug(
                        "SliderService: no userId provided, using admin fallback {UserId}",
                        user.Id);
                }
            }
            catch (Exception ex)
    ...
    /// </summary>
    private Task<List<BaseItem>> GetFallbackAnyAsync(SliderConfig cfg, Jellyfin.Data.Entities.User? user, CancellationToken ct)
    {
    ...
    private Task<List<BaseItem>> GetRandomAsync(SliderConfig cfg, Jellyfin.Data.Entities.User? user, CancellationToken ct)
    {
    ...
    private async Task<List<BaseItem>> GetFavouritesAsync(
        SliderConfig cfg, Jellyfin.Data.Entities.User? user, CancellationToken ct)
    {
    ...
    private Task<List<BaseItem>> GetCollectionsAsync(SliderConfig cfg, Jellyfin.Data.Entities.User? user, CancellationToken ct)
    {
        // v3.0.10: pass user when available - unscoped queries return [] in 10.10.
        var query = user is not null
            ? new InternalItemsQuery(user)
    ...
    private Task<List<BaseItem>> GetNewReleasesAsync(SliderConfig cfg, Jellyfin.Data.Entities.User? user, CancellationToken ct)
    {
