...
public class HomeService
{
    ...
    public async Task<IReadOnlyList<object>> BuildRailsAsync(Guid? userId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // v3.0.6: defend against persisted configs that come back from
            // XmlSerializer with Rails=null or Rails=[] (possible if an older
            // install wrote a config missing that element, or if the admin UI
            // posted an empty list). Without this we'd NRE on cfg.Rails.Where
            // or return an empty home silently. If the list is empty we fall
            // back to the ctor defaults so the user sees rails on fresh start.
            var railList = cfg.Rails ?? new List<HomeRail>();
            if (railList.Count == 0)
            {
                _logger.LogWarning(
                    "BuildRailsAsync: persisted Rails is empty, using defaults");
                railList = new HomeConfig().Rails;
            }
