...
public class HomeService
{
    ...
    public async Task<IReadOnlyList<object>> BuildRailsAsync(Guid? userId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // v3.0.8 defensive fix #2: empty Rails has TWO distinct meanings.
            //   (a) First-run before Plugin.SeedDefaultsIfNeeded persisted
            //       (HasBeenSeeded=false) — populate from defaults so the
            //       page never renders blank during the seed window.
            //   (b) User explicitly cleared the rails on the admin page
            //       (HasBeenSeeded=true)                             — respect
            //       their choice and render an empty home.
            // Earlier v3.0.8 fell through (a) for both, so a user who deleted
            // every rail saw defaults reappear permanently — confusing.
            var railList = cfg.Rails ?? new List<HomeRail>();
            if (railList.Count == 0)
            {
                var seeded = Plugin.Instance?.Configuration?.HasBeenSeeded ?? false;
                if (!seeded)
                {
                    _logger.LogInformation(
                        "BuildRailsAsync: persisted Rails empty AND HasBeenSeeded=false, applying first-run defaults");
                    railList = HomeConfig.DefaultRails();
                }
                else
                {
                    _logger.LogInformation(
                        "BuildRailsAsync: persisted Rails empty AND HasBeenSeeded=true, respecting user-cleared list (rendering empty home)");
                    return Array.Empty<object>();
                }
            }
            ...
            if (user is null)
            {
                try
                {
                    user = _userManager.Users.FirstOrDefault(u => u.HasPermission(PermissionKind.IsAdministrator))
                           ?? _userManager.Users.FirstOrDefault();
                    if (user is not null)
                    {
                        _logger.LogDebug(
                            "BuildRailsAsync: no userId, using admin fallback {UserId}",
                            user.Id);
                    }
                }
                catch (Exception ex)
    ...
    private object[] GetNewReleases(Jellyfin.Data.Entities.User? user, int max)
    {
    ...
    private object[] GetCategories(Jellyfin.Data.Entities.User? user, int max)
    {
    ...
    private async Task<object[]> GetTop10Async(HomeConfig cfg, Jellyfin.Data.Entities.User? user, string mediaType, int max, CancellationToken ct)
    {
        try
        {
    ...
    private object[] LocalTop10(Jellyfin.Data.Entities.User? user, string mediaType, int max)
    {
