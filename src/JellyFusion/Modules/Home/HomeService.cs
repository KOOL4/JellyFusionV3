...
public class HomeService
{
    ...
    private object[] GetStudiosRail()
    {
        ...
        // v3.0.16: also surface a click URL so bootstrap.js can navigate
        // to a list of items belonging to this studio. We use Jellyfin's
        // generic search route by name - it works on every install with
        // no need to look up the studio's internal Guid (which would
        // require a live IItemRepository query and could fail when the
        // user-configured studio name doesn't match a real Jellyfin
        // studio entity).
        return studios.Items
            ...
            .Select(s => (object)new
            {
                ...
                invert   = s.Invert,
                tags     = s.Tags,
                clickUrl = $"#/search.html?query={Uri.EscapeDataString(s.Name)}"
            })
            .ToArray();
    }
