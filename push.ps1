# ---------------------------------------------------------------
#  JellyFusion V3 v3.0.10  -  Force push + auto-verify
#
...
try {
    ...
    if ($versions -contains '3.0.10.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v3.0.10 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusionV3/releases/new?tag=v3.0.10'
} else {
    Write-Warning 'Remote manifest does not yet show v3.0.10.0 - raw cache is still warm.'
