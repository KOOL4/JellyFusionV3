# ---------------------------------------------------------------
#  JellyFusion V3 v3.0.4  -  Force push + auto-verify
#
...
Invoke-Git commit -m "JellyFusion v3.0.4 - hotfix over v3.0.3: (1) remove ClientScriptInjection gate from middleware so config round-trips can never disable bootstrap injection (banner was vanishing after Save because flag was not exposed in UI and came back false from XML persistence). (2) bootstrap shows real state in debug badge: 'banner N items | N rails' on success, 'no banner items' or 'ERR: msg' on failure - so feedback is visible without opening devtools. (3) MutationObserver on body re-renders home when SPA returns from config page after polling window (60s) has closed." | Out-Null

$localSha = (& git rev-parse HEAD).Trim()
Write-Host "    Commit SHA: $localSha"

Write-Host "==> Tag $tag (local)" -ForegroundColor Cyan
Invoke-Git tag -f -a $tag -m "JellyFusion v3.0.4 - all-in-one plugin" | Out-Null
...
try {
    ...
    if ($versions -contains '3.0.4.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v3.0.4 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusionV3/releases/new?tag=v3.0.4'
} else {
    Write-Warning 'Remote manifest does not yet show v3.0.4.0 - raw cache is still warm.'
