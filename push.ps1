# ---------------------------------------------------------------
#  JellyFusion V3 v3.0.3  -  Force push + auto-verify
#
...
Invoke-Git commit -m "JellyFusion v3.0.3 - hotfix over v3.0.2: strip Accept-Encoding before static file serve so ClientScriptInjectionMiddleware no longer decodes gzip bytes as UTF-8 (mojibake page). Safety net: skip injection if Content-Encoding is non-identity. Carries forward v3.0.2 fixes: IHttpResponseBodyFeature swap for SendFileAsync interception, surgical preview refresh, inflight re-entrancy guard." | Out-Null

$localSha = (& git rev-parse HEAD).Trim()
Write-Host "    Commit SHA: $localSha"

Write-Host "==> Tag $tag (local)" -ForegroundColor Cyan
Invoke-Git tag -f -a $tag -m "JellyFusion v3.0.3 - all-in-one plugin" | Out-Null
...
try {
    ...
    if ($versions -contains '3.0.3.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v3.0.3 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusionV3/releases/new?tag=v3.0.3'
} else {
    Write-Warning 'Remote manifest does not yet show v3.0.3.0 - raw cache is still warm.'
