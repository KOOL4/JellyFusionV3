# ---------------------------------------------------------------
#  JellyFusion V3 v3.0.5  -  Force push + auto-verify
#
...
Invoke-Git commit -m "JellyFusion v3.0.5 - CRITICAL hotfix over v3.0.4: remove runaway MutationObserver that froze the browser. (1) v3.0.4 added 'new MutationObserver(...).observe(document.body, {subtree:true})' to catch late home-container inserts after Save; but tryRenderHome itself appends DOM nodes (banner, rails, debug badge), each append fired the observer again, fired more fetches, and the page stayed black while the browser hung in two different browsers. Removed entirely. (2) Replaced with restartable poll: startPolling() is re-invoked from viewshow/hashchange/popstate handlers, so the 'user returns from configurationpage after 60s' case is still covered without touching the DOM globally. (3) Guard moved from a 'rendering' global flag to a 'data-jf-rendering' attribute on the container itself - when Jellyfin's SPA replaces the home container, the new one has no flag and renders cleanly, so duplicate concurrent fetches are prevented across poll ticks." | Out-Null

$localSha = (& git rev-parse HEAD).Trim()
Write-Host "    Commit SHA: $localSha"

Write-Host "==> Tag $tag (local)" -ForegroundColor Cyan
Invoke-Git tag -f -a $tag -m "JellyFusion v3.0.5 - all-in-one plugin" | Out-Null
...
try {
    ...
    if ($versions -contains '3.0.5.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v3.0.5 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusionV3/releases/new?tag=v3.0.5'
} else {
    Write-Warning 'Remote manifest does not yet show v3.0.5.0 - raw cache is still warm.'
