# ---------------------------------------------------------------
#  JellyFusion V3 v3.0.6  -  Force push + auto-verify
#
...
Invoke-Git commit -m "JellyFusion v3.0.6 - follow-up over v3.0.5: user reported debug badge showed 'no banner items | rails ERR' and no feature (banner, tag, studios) rendered. Fixes: (1) CONTROLLER: wrapped GetHomeRails and GetSliderItems in outer try/catch - both endpoints now return empty array instead of 500 on any exception, so 'rails ERR' can never appear again. Added INFO logging on slider call with mode/userId/count so the Jellyfin log shows why items are 0. (2) HOMESERVICE: full outer try/catch around BuildRailsAsync; null-safety on Plugin.Instance.Configuration.Home and cfg.Rails (covers case where XmlSerializer returns Rails=null after persisting an older config); GetUserById in its own try/catch so a bad userId claim doesn't abort the whole payload; default i18n titles for rails when rail.Title is null; showRank flag emitted so the bootstrap top-10 numbering actually shows. (3) SLIDERSERVICE: added GetFallbackAnyAsync - if Random/Favourites/Collections/New produces 0 items, fall back to an unfiltered broad pull of Movies+Series so the banner is NEVER empty on a library that has anything. Removed IsVirtualItem=false from GetRandomAsync which was silently filtering everything on some 10.10 installs. (4) PLUGIN_ID FIX (carried from v3.0.5 review): index.html previously shipped a1b2c3d4-e5f6-7890-abcd-ef1234567890 which did NOT match the plugin's real GUID b7c8d9e0-f1a2-3b4c-5d6e-7f8090a1b2c3. Every ApiClient.updatePluginConfiguration call was going to a 404 endpoint; save was only landing via the fetch fallback. Corrected to the real GUID in index.html, build.sh and .github/workflows/release.yml. (5) carries fixes of v3.0.5 (remove runaway MutationObserver, startPolling, data-jf-rendering guard)." | Out-Null

$localSha = (& git rev-parse HEAD).Trim()
Write-Host "    Commit SHA: $localSha"

Write-Host "==> Tag $tag (local)" -ForegroundColor Cyan
Invoke-Git tag -f -a $tag -m "JellyFusion v3.0.6 - all-in-one plugin" | Out-Null
...
try {
    ...
    if ($versions -contains '3.0.6.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v3.0.6 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusionV3/releases/new?tag=v3.0.6'
} else {
    Write-Warning 'Remote manifest does not yet show v3.0.6.0 - raw cache is still warm.'
